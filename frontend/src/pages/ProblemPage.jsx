...
const ProblemPage = () => {
  ...
  const { executeCode, testCode, submission, testResults, isExecuting, isSubmitting } = useExecutionStore();
  ...
  const handleRunCode = (e) => {
    ...
    try {
      ...
      testCode(code, language_id, stdin, expected_outputs);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <EnhancedCodeEditor
            ...
            onLanguageChange={(lang) => {
              ...
              const mappedLang = upperLang === 'CPP' ? 'CPP' : upperLang;
              setSelectedLanguage(mappedLang);
              setCode(problem.codeSnippets?.[mappedLang] || '');
            }}
            onRun={handleRunCode}
            ...
            {submission ? (
              ...
              <>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className={`badge ${testResults.allPassed ? 'badge-success' : 'badge-error'} badge-lg`}>
                  ...
                  <table className="table table-zebra w-full">
                    ...
                    <tbody>
                      {testResults.results.map((result, index) => (
                        <tr key={index} className={result.passed ? 'text-success' : 'text-error'}>
                          ...
                          <td className="text-sm">{result.time || 'N/A'}</td>
                          <td className="text-sm">{result.memory || 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {/* AI Chatbot */}
        <Chatbot problem={problem} userCode={code} />
      </div>
    </div>
  );
};
