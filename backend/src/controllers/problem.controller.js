...
export const createProblem = async (req, res) => {
  ...
  try {
    ...
    // Skip validation if Judge0 is not available (for development)
    const skipValidation = process.env.SKIP_JUDGE0_VALIDATION === 'true';
    
    if (!skipValidation) {
      console.log('🔍 Validating reference solutions with Judge0...');
      ...
      for (const [language, solutionCode] of Object.entries(referenceSolutions)) {
        ...
        try {
          ...
          console.error('Judge0 API Error:', judgeError.message);
          return res.status(400).json({
            error: 'Judge0 API is not available. Please ensure Judge0 is running on the configured URL.',
            details: judgeError.message,
            suggestion: 'Set SKIP_JUDGE0_VALIDATION=true in .env to skip validation during development'
          });
        }
      }
    } else {
      console.log('⚠️  Skipping Judge0 validation (SKIP_JUDGE0_VALIDATION=true)');
    }
